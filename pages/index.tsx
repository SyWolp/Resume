...
const Home: NextPage = () => {
  ...
  return (
    <div className='w-full h-screen bg-yellow-100'>
      <Header title="이력서 선택" main={false} />
      ...
      <div onClick={()=> setSelect(false)} className={`${select ? selectButton[1] : selectButton[0]} inline-block w-32 rounded-t-xl text-center cursor-pointer`}>내 이력서</div>
        ...
        <>        
          <Link href={"./firstPage"}>
            <a className='w-1/6 h-2/3 bg-red-200 mx-10 my-14 shadow-2xl rounded-3xl'>
              <div className='text-center leading-[13rem]'>1번 스타일</div>
            </a>
          </Link>
          <Link href={"./firstPage"}>
            <a className='w-1/6 h-2/3 bg-red-500 mx-10 my-14 shadow-2xl rounded-3xl'>
              <div className='text-center leading-[13rem]'>2번 스타일</div>
            </a>
          </Link> 
        </> : 
        <div>abc</div>
